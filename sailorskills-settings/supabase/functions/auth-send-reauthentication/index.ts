/**
 * Supabase Edge Function: Custom Auth Reauthentication Email
 * Replaces Supabase's built-in reauthentication email to add BCC support
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const subject = 'Reauthentication Required - Sailor Skills';
    const html = `<!DOCTYPE html><html><body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;"><h1 style="color: #1e3a8a;">Verify Your Identity</h1><p>For security purposes, please verify your identity by clicking the button below:</p><a href="${confirmation_url}" style="display: inline-block; background: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; margin: 20px 0;">Verify Identity</a><p style="color: #666; font-size: 14px;">This link will expire in 15 minutes.</p><p style="color: #666; font-size: 12px;">© ${currentYear} Sailor Skills Marine Services. All rights reserved.</p></body></html>`;

    const result = await sendEmail(email, subject, html);

    await supabase.from('email_logs').insert({
      email_type: 'reauthentication',
