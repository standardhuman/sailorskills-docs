...
export async function createTestServiceLog(overrides = {}) {
  ...
  const serviceLog = await queryOne(
    `INSERT INTO service_logs (customer_id, service_type, status, is_test, test_scenario_id)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [data.customer_id, data.service_type, data.status, data.is_test, data.test_scenario_id]
  );
...
export async function createTestInvoice(overrides = {}) {
  ...
  const invoice = await queryOne(
    `INSERT INTO invoices (customer_id, amount, status, is_test, test_scenario_id)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [data.customer_id, data.amount, data.status, data.is_test, data.test_scenario_id]
  );
...
export async function cleanupTestData(customerId) {
  ...
  await queryDatabase('DELETE FROM invoices WHERE customer_id = $1', [customerId]);
  await queryDatabase('DELETE FROM service_logs WHERE customer_id = $1', [customerId]);
...
export async function cleanupAllTestData() {
  ...
  await queryDatabase('DELETE FROM invoices WHERE test_scenario_id = $1', [testRunId]);
  await queryDatabase('DELETE FROM service_logs WHERE test_scenario_id = $1', [testRunId]);
  await queryDatabase('DELETE FROM boats WHERE test_scenario_id = $1', [testRunId]);
  await queryDatabase('DELETE FROM customers WHERE test_scenario_id = $1', [testRunId]);
}
